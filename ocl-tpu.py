...
def camThread(label, results, frameBuffer, camera_width, camera_height, vidfps, cam_num):
    ...
    window_name = "Camera"
    cv2.UMat(cv2.namedWindow(window_name, cv2.WINDOW_AUTOSIZE))
    ...
    while True:
        ...
        cv2.UMat(cv2.imshow('Camera', imdraw))
...
def overlay_on_image(frames, object_infos, label, camera_width, camera_height):
    ...
    for obj in object_infos:
        ...
        cv2.UMat(cv2.rectangle(img_cp, (box_left, box_top), (box_right, box_bottom), box_color, box_thickness))
        ...
        cv2.UMat(cv2.rectangle(img_cp, (label_left - 1, label_top - 1), (label_right + 1, label_bottom + 1), label_background_color, -1))
        cv2.UMat(cv2.putText(img_cp, label_text, (label_left, label_bottom), cv2.FONT_HERSHEY_SIMPLEX, 0.5, label_text_color, 1))

    cv2.UMat(cv2.putText(img_cp, fps,       (camera_width-170,15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (38,0,255), 1, cv2.LINE_AA))
    cv2.UMat(cv2.putText(img_cp, detectfps, (camera_width-170,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (38,0,255), 1, cv2.LINE_AA))
